...
// Normalise un titre pour la comparaison (ignore ponctuation, casse, numéros)
...
function trouverCorrespondance(chapitreImporté, nœuds) {
  ...
  let match = nœuds.find(n => normaliser(n.titre) === titreN);
  if (match) return match;

  // Correspondance partielle — 3 mots en commun minimum
  const motsImporté = new Set(titreN.split(" ").filter(m => m.length > 3));
  let meilleurScore = 0;
  let meilleur = null;

  for (const n of nœuds) {
    const motsN = normaliser(n.titre).split(" ").filter(m => m.length > 3);
    const communs = motsN.filter(m => motsImporté.has(m)).length;
    if (communs > meilleurScore) {
      meilleurScore = communs;
      meilleur = n;
    }
  }
