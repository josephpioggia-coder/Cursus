/**
 * IMPORT COMPLET — Cycle + Texte Tome I
 * Crée les 4 projets ET importe le texte en une seule opération.
 * Aucun doublon possible.
 * Usage : node import-tout.mjs MON_MOT_DE_PASSE
 */
...
async function main() {
  if (!PASSWORD) {
    console.error("❌ Usage : node import-tout.mjs MON_MOT_DE_PASSE");
    process.exit(1);
  }
  ...
  if (errAuth) { console.error("❌", errAuth.message); process.exit(1); }
  const userId = auth.user.id;
  console.log(`✓ Connecté : ${auth.user.email}\n`);

  // Charger les textes extraits du Tome I
  let textes = {};
  try {
    textes = JSON.parse(readFileSync('./textes_tome1.json', 'utf8'));
    console.log(`📄 ${Object.keys(textes).length} textes de chapitres chargés\n`);
  } catch(e) {
    console.log("⚠️  Fichier textes_tome1.json non trouvé — structure créée sans texte\n");
  }

  let totalProjets = 0, totalParties = 0, totalChapitres = 0, totalMots = 0;

  for (const tome of CYCLE) {
    console.log(`📖 ${tome.titre}`);

    const { data: projet, error: errP } = await supabase
      .from("projets")
      .insert([{
        user_id: userId,
        titre: tome.titre, genre: tome.genre, statut: tome.statut,
        couleur: tome.couleur, objectif_mots: tome.objectif_mots,
    ...
    for (let pi = 0; pi < tome.parties.length; pi++) {
      ...
      const { data: nPartie, error: errPa } = await supabase
        .from("noeuds")
        .insert([{
          projet_id: projet.id, parent_id: null,
          type: "partie", titre: partie.titre,
          ordre: pi + 1, texte: "",
        }])
        .select().single();

      if (errPa) { console.error(`  ❌ ${errPa.message}`); continue; }
      totalParties++;
      console.log(`  📂 ${partie.titre}`);

      for (let ci = 0; ci < partie.chapitres.length; ci++) {
        const titreChapitre = partie.chapitres[ci];

        // Cherche le texte correspondant
        const texteHTML = textes[titreChapitre]?.html || "";
        const mots = textes[titreChapitre]?.mots || 0;

        const { error: errC } = await supabase
          .from("noeuds")
          .insert([{
            projet_id: projet.id, parent_id: nPartie.id,
            type: "chapitre", titre: titreChapitre,
            ordre: ci + 1, texte: texteHTML,
          }]);
        ...
        if (!errC) {
          ...
          totalMots += mots;
          if (mots > 0) {
            console.log(`    📄 ${titreChapitre.slice(0, 45)} — ${mots} mots ✓`);
          } else {
            console.log(`    📄 ${titreChapitre.slice(0, 45)}`);
          }
        }
      }
    }
  ...
  console.log(`📝 ${totalMots.toLocaleString("fr-FR")} mots importés dans le Tome I`);
