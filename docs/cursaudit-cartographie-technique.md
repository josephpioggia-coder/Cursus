...
| # | Chantier | Statut |
|---|---|---|
| 1 | Architecture/code — composants partagés (moteur IA, questionnaire, UI) | Bien avancé : schéma `audits`/`audit_sections`/`audit_pricing_rules`/`audit_criteria` (30 critères) appliqué, questionnaire de qualification CursAudit figé, **protocole 60805-06 déployé, testé en conditions réelles, et intégré dans l'éditeur (onglet "Vérification" de CopiloteIA)** — chantier Cursus Édition essentiellement bouclé. **Moteur d'analyse CursAudit (`analyser-unite-cursaudit`) écrit le 16/08/2026** — traite une unité à la fois (limite de temps d'exécution d'une Edge Function), modes "1 IA"/"2 IA" implémentés, "confrontation ciblée"/"arbitrage dialogique" pas encore ; orchestration multi-unités, import de document et paiement Stripe pour CursAudit restent à construire ; questionnaire côté Cursus Édition (b) et composants UI partagés (c) toujours pas démarrés |
