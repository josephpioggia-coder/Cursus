...
| # | Chantier | Statut |
|---|---|---|
| 1 | Architecture/code — composants partagés (moteur IA, questionnaire, UI) | Bien avancé le 15/08/2026 : module `moteur-ia-structure.ts` écrit, schéma `audits`/`audit_sections`/`audit_pricing_rules`/`audit_criteria` (30 critères) appliqué, questionnaire de qualification CursAudit figé, **Edge Function `verification-deux-ia` (protocole 60805-06) écrite et déployée sur Supabase** (pas encore testée en conditions réelles, intégration UI manquante) ; questionnaire côté Cursus Édition (b) et composants UI (c) toujours pas démarrés |
