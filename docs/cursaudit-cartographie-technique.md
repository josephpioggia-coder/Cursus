...
| # | Chantier | Statut |
|---|---|---|
| 1 | Architecture/code — composants partagés (moteur IA, questionnaire, UI) | Bien avancé : schéma `audits`/`audit_sections`/`audit_pricing_rules`/`audit_criteria` (30 critères) appliqué, questionnaire de qualification CursAudit figé, **Edge Function `verification-deux-ia` (protocole 60805-06) déployée et testée avec succès en conditions réelles le 16/08/2026** (premier appel réel sur un vrai manuscrit, verdict cohérent, dialogue à profondeur adaptative confirmé) — intégration UI dans l'éditeur restant à faire ; questionnaire côté Cursus Édition (b) et composants UI (c) toujours pas démarrés |
