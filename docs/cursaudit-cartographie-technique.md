...
| # | Chantier | Statut |
|---|---|---|
| 1 | Architecture/code — composants partagés (moteur IA, questionnaire, UI) | Bien avancé : schéma `audits`/`audit_sections`/`audit_pricing_rules`/`audit_criteria` (30 critères) appliqué, questionnaire de qualification CursAudit figé, **protocole 60805-06 déployé, testé en conditions réelles, et intégré dans l'éditeur (onglet "Vérification" de CopiloteIA)** — chantier Cursus Édition essentiellement bouclé. **Moteur d'analyse CursAudit (`analyser-unite-cursaudit`) écrit et testé avec succès en conditions réelles le 16/08/2026** — mode "1 IA" validé sur une unité réelle (8 critères du palier Essentiel, sortie ancrée dans le texte, correctement diagnostiquée comme littéraire/non-factuelle) ; traite une unité à la fois (limite de temps d'exécution d'une Edge Function), mode "2 IA" écrit mais pas encore testé, "confrontation ciblée"/"arbitrage dialogique" pas implémentés . **Orchestrateur multi-unités (`orchestrer-audit-cursaudit`, référence 60816-01) écrit le 16/08/2026** — traite les unités non encore analysées d'un audit par lot borné dans le temps (25s de marge, pas un compte fixe), marque les échecs par unité plutôt que de les retenter en boucle, bascule `audits.statut` brouillon→paye→en_traitement→termine ; pas de tâche de fond automatique, c'est l'appelant qui doit rappeler tant que `restantes > 0`. **Testé avec succès en conditions réelles le 16/08/2026** : lot de 3 unités traitées en un appel (dont une réinsérée après un premier essai à vide, bug corrigé — voir note dans le fichier source), `audits.statut` correctement basculé jusqu'à "termine". Contraste qualitatif confirmé entre unités : un passage factuel non sourcé (chiffre précis attribué à "une étude publiée en 2019" sans auteur ni revue) a été diagnostiqué "besoin de preuve fort", "à sourcer", "risque d'influence élevé" — contre "recevable"/"besoin de preuve faible" pour les passages narratifs du même lot. **Segmentation + création d'audit (`segmenterCursAudit.js`, `auditsAPI.créerDepuisTexte`) écrites le 16/08/2026** — texte collé → unités → `audits` (statut "brouillon") + `audit_sections`, testé unitairement en local (segmentation seule), pas encore testé en conditions réelles ni relié à une interface (aucun bouton n'existe encore dans l'app). Ne gère que du texte déjà en clair — l'extraction depuis un fichier `.docx`/`.pdf` reste à construire (reprendre la logique JSZip déjà éprouvée dans `ImportDocx.jsx`, pas `mammoth` qui est une dépendance présente mais inutilisée dans ce dépôt). **Page de création (`CursAudit.jsx`, nouvelle entrée "CursAudit" dans la navigation) écrite le 16/08/2026** — texte collé, palier (3 fixes, "Libre" non proposé), mode IA (seuls "1 IA"/"2 IA", les deux implémentés), format de rapport, prix calculé en direct (`tarifCursAudit.js`, à partir du nombre réel d'unités segmentées et de `audit_pricing_rules` ; multiplicateur commercial simplifié en une valeur fixe par palier, pas encore la grille par tranches complète). Crée l'audit en statut "brouillon" — pas de bouton de paiement, puisqu'aucun flux Stripe CursAudit n'existe. Compile sans erreur, **pas encore vue dans le navigateur** (nécessite de fusionner la branche ou un aperçu Vercel de la PR). Paiement Stripe pour CursAudit reste entièrement à construire ; questionnaire côté Cursus Édition (b) et composants UI partagés (c) toujours pas démarrés |
